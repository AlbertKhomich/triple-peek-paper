...
We tested \tp{} with public Wikidata and DBpedia endpoints, the public SemRepo endpoint, and DICE-hosted endpoints exposing Wikidata, DBpedia, and a university corpus (Uni corpus). SemRepo describes research software and its scholarly context~\cite{semrepo2026}. The tested deployments included Virtuoso, Tentris, and Apache Jena Fuseki. These exploratory checks extend the evidence beyond the bundled example to multiple datasets and endpoint implementations; they do not constitute a complete query-form or backend compatibility matrix.

\paragraph{Large-catalog experience.}
In an exploratory test, we loaded a catalog of 20 million entities, with each entity represented by one row containing labels and aggregated types. Interactive search remained responsive during manual use. This is a qualitative observation, not a measured response-time bound: no controlled timing data, hardware specification, query workload, or concurrency profile are reported here. The observation concerns local catalog search and does not establish the latency of live SPARQL actions. Catalog size here counts entities after aggregation, rather than triples or duplicate input rows.
