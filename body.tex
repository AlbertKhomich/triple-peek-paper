...
\emph{Quick Start:} clone the repository and follow its documented Docker Compose setup using the bundled 10,000-entity catalog to reproduce the live demonstration. \emph{Large-catalog experiment:} download the archived 17,795,730-entity CSV from Zenodo,\footnote{\url{https://zenodo.org/records/22958238}.} import it through the documented validation and PostgreSQL import pipeline, and run the local search workload reported below. For the capture with known query strings, issue searches in the listed order with \texttt{limit=20}, using localhost and browser caching disabled. The two paths reproduce different components: the live visitor workflow and local search over a loaded large catalog. Neither requires timing external SPARQL services.


\paragraph{Experimental environment and scope.}
All deployment tests and search measurements were conducted on 25 September 2026 on a VMware VM with 8 vCPUs (Intel Xeon Platinum 8462Y+), 31\,GiB RAM, and a 1\,TB virtual disk with an ext4 filesystem. TriplePeek and PostgreSQL ran in separate Docker Compose containers on this VM. We assess local catalog search and functional compatibility of query buttons and catalog generation. External endpoint response times are outside this evaluation. Search accesses PostgreSQL independently of the configured endpoint; changing that endpoint with the catalog and search configuration fixed does not require repeating the search measurements.
...
The local catalog held 17,795,730 entities, one row per entity with labels and aggregated types. While typing \emph{Berlin place}, six browser Network-panel search-request durations were 502, 52, 42, 83, 184, and 124\,ms (range 42--502\,ms; median 103.5\,ms). These HTTP durations measure the search service through the browser--application exchange, including catalog retrieval; they exclude the 300\,ms pre-request debounce and do not isolate SQL execution. They sample one typing session, not repeated identical queries. Measurements used localhost with browser caching disabled. An additional Network-panel capture shows six successful HTTP 200 search requests with \texttt{limit=20}: \texttt{neusch} (142\,ms), \texttt{italy} (117\,ms), \texttt{italy cast} (114\,ms), \texttt{neusch} (88\,ms), \texttt{muse} (129\,ms), and \texttt{museum} (48\,ms). This capture spans 48--142\,ms (median 115.5\,ms). Both sets characterize observed local search-service performance on an already loaded catalog; neither involves SPARQL execution. Browser cache disabling does not imply cold PostgreSQL or operating-system caches. These small samples do not establish throughput or a latency distribution under concurrent load.
