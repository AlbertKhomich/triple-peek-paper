...
3. The six-endpoint inventory, three Tentris 1.0.0 versions, DICE snapshots, DESCRIBE coverage, VMware environment, and six search timing observations have now been supplied. Tests were conducted on 25 September 2026 on 8 vCPUs (Intel Xeon Platinum 8462Y+), 31 GiB RAM, and a 1 TB ext4 virtual disk. Still document exact query strings for each sample, cache state, concurrency, and the catalog source. Record the endpoint/action mapping for custom queries and identify the earlier Uni corpus/Fuseki deployment before asserting a complete compatibility matrix.
